...
import time
from filters import *
from map_calculator import calculate_map

print("所有模块导入成功")

exp_folder = os.path.join(args.exp_dir, 'exp_{}'.format(args.exp_num))
print("实验文件夹路径已设置 : {exp_folder}")


print("--- SCRIPT STARTED IN TERMINAL ---")
...
class YoloTest(object):
    def __init__(self):
        #输入：它没有外部输入，而是从cfg这个配置文件中读取所有必需的“办案参数”。

        #输出：它不返回任何东西，但会把“侦探”的所有装备和大脑都准备好，存放在self.的各个属性里。
        print("yolo初始化开始")
        ...
        print("    --- 7. 正在从文件中恢复模型... (这是最可能出问题的步骤) ---")
    ...
    def predict(self, image, image_name):
        ...
        image_data, _ = utils.image_preporcess(image, [self.input_size, self.input_size])
        ...
        #为什么要汇集到一起？—— 为了解决“重复上报”的问题
        #最关键的原因是：不同的小队（不同尺度的检测头），很可能会对同一个目标进行重复上报
        #总而言之，拼接是为了全局筛选。因为不同尺度的检测头可能会对同一个物体产生重叠的预测，我们必须将所有预测放在一个平面上进行比较和去重（NMS），才能得到干净、准确的最终结果。
        ...
        if self.isp_flag:
            ...
            image_isped,_ = utils.image_unpreporcess(image_isped[0, ...], [org_h, org_w])
    ...
    def evaluate(self):
        ...
        with open(self.annotation_path, 'r') as annotation_file:
            for num, line in enumerate(annotation_file):
                ...
                if self.write_image:
                    if self.isp_flag:
                        image = utils.draw_bbox(image, bboxes_pr, self.classes, show_label=self.show_label)
                    else:
                        image = utils.draw_bbox(image, bboxes_pr, self.classes, show_label=self.show_label)
        ...
        print("\n--- 所有图片预测完成，开始进行mAP计算 ---")
        mAP, APs_by_class, APs_by_size  = calculate_map(ground_truth_dir_path ,predicted_dir_path)
        print("--- mAP计算完成 ---")

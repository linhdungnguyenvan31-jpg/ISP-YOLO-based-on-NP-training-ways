...
def image_preporcess(image, target_size, gt_boxes=None):
    ...
    if gt_boxes is None or len(gt_boxes) == 0:
        return image_paded,np.array([])  # 同时返回图片和一个空的标注框数组
...
def image_unpreporcess(image, target_size, gt_boxes=None):
    """
    【最终完美版】反处理函数：使用与原作者相同的3参数接口，但内部实现了正确的反处理逻辑。
    """
    input_h, input_w, _ = image.shape
    original_h, original_w = target_size

    # --- 恢复图像 (Image Restoration) ---
    scale = min(input_w / original_w, input_h / original_h)
    nw, nh = int(scale * original_w), int(scale * original_h)
    dw, dh = (input_w - nw) // 2, (input_h - nh) // 2
    image = np.clip(image * 255, 0, 255).astype(np.uint8)
    image_bgr = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
    unpadded_image = image_bgr[dh:nh+dh, dw:nw+dw, :]
    restored_image = cv2.resize(unpadded_image, (original_w, original_h))

    # --- 转换坐标 (Coordinate Transformation) ---
    if gt_boxes is None or len(gt_boxes)==0:
        return restored_image, np.array([])
    
    gt_boxes[:, [0, 2]] = (gt_boxes[:, [0, 2]] - dw) / scale
    gt_boxes[:, [1, 3]] = (gt_boxes[:, [1, 3]] - dh) / scale
    
    gt_boxes[:, 0] = np.maximum(0, gt_boxes[:, 0])
    gt_boxes[:, 1] = np.maximum(0, gt_boxes[:, 1])
    gt_boxes[:, 2] = np.minimum(original_w, gt_boxes[:, 2])
    gt_boxes[:, 3] = np.minimum(original_h, gt_boxes[:, 3])

    restored_bboxes = gt_boxes.astype(np.int32)
    return restored_image, restored_bboxes
